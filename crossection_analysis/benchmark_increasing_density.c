...
int main() {
    ...
    fprintf(log_prim,"density,time\n");
    fprintf(log_kruskal,"density,time\n");
    fprintf(log_re_del,"density,time\n");
    ...
    for (int i = 0; i < N; i++){
        ...
        density_fix += 0.001;          //sparse or dense graph
        E[i] = V - 1 + ((int)(density_fix * max_edges - (V - 1) + 1));
        density[i] = (float)E[i] /(float) max_edges; //actual density

        // if (i > 0) {
        //     while (E[i] <= E [i - 1] && density[i - 1] < 1){
    ...
    for (int j = 0; j < N; j++){
        ...
        for (int m = 0; m < iterations; m++){
            ...
            time_prim += time_function(primMST, graphs[j], &msts[j], label[k], j);
            displayGraph(msts[j]);
            printf("\n");
            k++;
            ...
            time_kruskal += time_function(kruskalMST, graphs[j], &msts[j], label[k], j);
            displayGraph(msts[j]);
            printf("\n");
            k++;
            ...
            // time_re_del += time_function(reverseDeleteMST, graphs[j], &msts[j], label[k], j); // = time_function(...)
            // displayGraph(msts[j]);
            // printf("\n");
        }

        fprintf(log_prim,"%.6f, %.6f\n",density[j], time_prim/iterations);
        fprintf(log_kruskal,"%.6f,%.6f\n", density[j], time_kruskal/iterations);
        // fprintf(log_re_del,"%d,%.6f\n", j, time_re_del/iterations);
    }
